...
def multiimport0():
    ...
    for record in text_list:
        ...
        import_tree.insert(parent='', index='end', iid=count, text=path.name, values=record)
...
def export_totxt():
    try:
        ...
        for i in range(len(df_all)):
            ...
            df_all[i].to_csv(item_text[i][0][:-4] + ".txt", index=False, sep='\t')
...
def popup():
    ...
    ok_button = tk.Button(mframe, text="Close this window", command=pop.destroy,font=('helvetica', 12, 'bold'), width=15)
    show_folder = tk.Button(mframe, text="Show folder", command=openfile,font=('helvetica', 12), width=15)
...
browseButton = tk.Button(fixframe, text="Import .mpr files", command=multiimport0, font=('helvetica', 12, 'bold'), width=15)
exportButton_xlsx = tk.Button(fixframe, text="Export as .xlsx file", command=export_toexcel, font=('helvetica', 12, 'bold'), width=15)
exportButton_CSV = tk.Button(fixframe, text="Export as .csv file", command=export_tocsv, font=('helvetica', 12, 'bold'), width=15)
exportButton_txt = tk.Button(fixframe, text="Export as .txt file", command=export_totxt, font=('helvetica', 12, 'bold'), width=15)
